...
var GH_LAYOUTS_DIR = 'legacy/biblioteca/data/layouts';
var GH_BIBLIOTECA_MANIFEST_PATH = 'legacy/biblioteca/data/manifest.js';
var GH_COPY_BASE_TEMPLATE_PATH = 'app/features/biblioteca/controllers/copy-base-template.js';
    ...
    for (var i = 0; i < layouts.length; i++) {
      ...
      if (entry && typeof entry === 'object' && entry.file && String(entry.id || '').toLowerCase() === normalizedId) {
        return {
          manifestInfo: info,
          entry: entry,
          path: 'legacy/biblioteca/data/' + entry.file
        };
      }
    }
    return null;
...
function githubSaveNewLayout(fileName, objectCode, layoutId) {
  ...
  var individualPath = 'legacy/biblioteca/data/' + individualFile;
  var nextContent;
...
function initSenkoBibliotecaGithubV2() {
  ...
  /* Bot̵es de variante (nova e editar) foram movidos para
     legacy/biblioteca/github/senko-github-variants.js */
