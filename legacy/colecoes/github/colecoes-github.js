...
var COL_GH_MANIFEST_PATH = 'legacy/colecoes/data/manifest.js';
...
function colGhFilePath(slug) {
  ...
  return 'legacy/colecoes/data/collections/' + normalizedSlug + '/collection.js';
}
    ...
    for (var i = 0; i < layouts.length; i++) {
      ...
      if (
        ...
        return {
          ...
          path: 'legacy/colecoes/data/' + layoutEntry.file
        };
      }
    }
    return null;
...
function colGhGetCollectionLayoutFiles(slug) {
  ...
  return colGhReadManifest().then(function (info) {
    ...
    return entry.layouts
      ...
      .map(function (layoutEntry) { return 'legacy/colecoes/data/' + layoutEntry.file; });
  }).catch(function () {
    return [];
  });
}
...
function colGhFlushGroups() {
  ...
  return colGithubGetFile('legacy/colecoes/data/col-groups-data.js').then(function (data) {
    return colGithubPutFile('legacy/colecoes/data/col-groups-data.js', content, data.sha, '[Colecoes] sync groups');
  }).catch(function (error) {
    if (error.message && error.message.indexOf('404') !== -1) {
      return colGithubPutFile('legacy/colecoes/data/col-groups-data.js', content, null, '[Colecoes] create groups file');
    }
    throw error;
  }).then(function () {
...
function colGhGetFile(slug) {
  ...
  return colGhReadManifest().then(function (info) {
    ...
    if (found.entry && typeof found.entry === 'object' && found.entry.file) {
      return 'legacy/colecoes/data/' + found.entry.file;
    }
...
function colGhAddLayout(slug, layoutData) {
  ...
  var layoutPath = 'legacy/colecoes/data/collections/' + normalizedSlug + '/layouts/' + layoutData.id + '.js';
