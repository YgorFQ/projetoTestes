...
assert.match(view, /id="team-notes-title-status"/);
assert.match(view, /id="team-notes-section-status"/);
...
assert.match(script, /showInlineStatus\('title', 'success'/);
assert.match(script, /showInlineStatus\('title', 'duplicate'/);
assert.match(script, /showInlineStatus\('section', 'duplicate'/);
assert.doesNotMatch(script, /setToast\('Página salva no Firebase/);
...
assert.match(styles, /animation:\s*team-notes-workspace-status-fade 2s/);
assert.match(styles, /\.team-notes-workspace-inline-status\.is-duplicate/);
...
assert.equal(fs.existsSync(path.join(root, 'app/tools/team-notes/data/manifest.js')), false);
assert.equal(fs.existsSync(path.join(root, 'app/tools/team-notes/data/notes/teste.js')), false);
